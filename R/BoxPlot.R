...
BoxPlot <- function(
  ...
  if(is.null(dim(x))) {
    bp$stats <- sapply(x, FUN = stats::quantile, probs = whiskers, na.rm = TRUE)
  } else {
    bp$stats <- apply(
      x, MARGIN = 2, FUN = stats::quantile, probs = whiskers, na.rm = TRUE
    )
    bp$names <- colnames(x)
  }
  graphics::bxp(
    bp, outline = FALSE, show.names = TRUE, axes = FALSE, xaxs = 'i',
    pars = list(

# From https://github.com/metrumresearchgroup/slickR/R/getHelp.R
# https://cran.r-project.org/package=slickR
...
# tmp <- getHelp("SideBySideDensity", "Barbouille")
tmp <- getHelpFile(index.search("SideBySideDensity", find.package("Barbouille"), TRUE))
lst <- sapply(lapply(tmp, attributes), "[[", "Rd_tag")
lst <- lst[lst != "\\usage"]
txt <- RdTextFilter(tmp, drop = lst, keep = "RCODE")
txt <- gsub("^ *$", "", txt, perl = T)
txt <- txt[txt != ""]
